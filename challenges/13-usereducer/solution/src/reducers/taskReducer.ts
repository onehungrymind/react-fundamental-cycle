...
export type TaskAction =
  ...
  | {
      ...
      payload: {
        ...
        // completedAt is generated outside the reducer by the component and
        // passed here so the reducer stays pure (no Date.now() inside).
  ...
  switch (action.type) {
    ...
    case 'UPDATE_STATUS': {
      ...
      return state.map((task) => {
        ...
        // Enforce the state machine — ignore invalid transitions silently
        ...
        return {
          ...task,
          status: newStatus,
          // Only set completedAt when transitioning to Done; preserve existing
          // value for all other transitions (caller passes undefined otherwise)
    ...
    case 'UNASSIGN_TASK': {
      ...
      return state.map((task) => {
        if (task.id !== taskId) return task;
        // Spread then omit assigneeId using destructuring
