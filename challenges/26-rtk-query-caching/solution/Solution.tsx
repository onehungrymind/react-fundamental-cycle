...
const api = createApi({ reducerPath:'api', baseQuery: fetchBaseQuery({ baseUrl: '/api' }), endpoints: b=>({ items: b.query<any[], void>({ query: ()=> 'items' }) }) });
...
function List(){ const { data, isLoading } = useItemsQuery(); if(isLoading) return <p>Loading…</p>; return <ul>{data?.map(i=><li key={i.id}>{i.title}</li>)}</ul>; }
