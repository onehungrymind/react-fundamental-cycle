...
export default function Solution(){
  ...
  React.useEffect(()=>{ if(!sel) return; (async()=>{ const r=await fetch('/api/items'); const all=await r.json(); setDetail(all.find((x:any)=>x.id===sel)); })(); },[sel]);
  return <div><ul>{items.map(i=><li key={i.id}><button onClick={()=>setSel(i.id)}>{i.title}</button></li>)}</ul><div>{detail?.title ?? 'none'}</div></div>;
}
