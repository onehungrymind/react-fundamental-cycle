...
const slice = createSlice({
  name: "mdv",
  initialState: { items: [], status: "idle" as "idle" | "loading" | "done" },
  reducers: {},
  extraReducers: (b) => {
...
function List() {
  const d = useDispatch();
  const st = useSelector((s: Root) => s.status);
  const items = useSelector((s: Root) => s.items);
  React.useEffect(() => {
    d(fetchItems() as any);
  }, [d]);
  if (st === "loading") return <p>Loading…</p>;
  return (
    <ul>
      {items.map((i: any) => (
