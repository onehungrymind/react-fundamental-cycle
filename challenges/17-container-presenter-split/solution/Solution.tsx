...
function ListP({ items, sel, onSelect }: { items: Item[]; sel?: number; onSelect: (id: number) => void }) {
  return (
    <ul>
      {items.map(i => (
        <li key={i.id}>
...
function DetailP({ item }: { item?: Item }) {
  return <div>{item ? item.t : 'none'}</div>;
}
...
export default function Solution() {
  ...
  return (
    <div>
      <ListP items={items} sel={sel} onSelect={setSel} />
      <DetailP item={item} />
    </div>
  );
}
