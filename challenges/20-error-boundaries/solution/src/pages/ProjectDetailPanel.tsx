...
//
// ProjectDetailContent is wrapped in an ErrorBoundary.
//
// Isolation effect: loading states and API error handling (isPending, error)
// live outside the boundary in ProjectDetailPanel, so a crash in the
// rendered project content doesn't prevent the loading/error paths from
// working.  If ProjectDetailContent throws, only the content area is
// replaced by the fallback.  The sidebar still works.
//
// Why wrap ProjectDetailContent and not the whole ProjectDetailPanel?
// The isPending/error guard clauses live before the ErrorBoundary.
// TanStack Query's error state is handled via ErrorMessage (not boundary).
// The boundary is for unexpected render errors (e.g. null-deref in JSX).
