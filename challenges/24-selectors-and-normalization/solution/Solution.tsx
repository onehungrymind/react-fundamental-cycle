...
type NormalizedState = { byId: Record<number, Item>; ids: number[]; sel: number | undefined };

const initial: NormalizedState = { 
  byId: {1:{id:1,t:'A'},2:{id:2,t:'B'}}, 
  ids:[1,2], 
  sel:1 
};
...
export default function Solution(){ 
  ...
  const sel=useSelector(selectSelected); 
  return <div><ul>{items.map(i=><li key={i.id}>{i.t}</li>)}</ul><div>{sel?.t ?? 'none'}</div></div>; 
}
